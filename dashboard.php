...
if (!$events) {
    $noEvents = 'Non ci sono eventi, creane uno!';
}

if (isset($_GET['password_reset']) && $_GET['password_reset'] === 'success') {
    echo '<script>alert("Password cambiata con successo, sei appena stato reinderizzato in dashboard");</script>';
}
?>
...
<?php 
  ...
  ?>
    <main>
        ...
        <?php }else{ ?>
            <h1 id="title"><?php echo $noEvents ?></h1>
        <?php }?>
    </main>
